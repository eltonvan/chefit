...
def count(request):
    try:
        if request.user.customer.membership:
            return render(request, "plans/count.html")
...
def checkout(request):
    try:
        ...
        pass
    coupons = {"welcome": "10", "starter": "23", "new-year": "15"}
    if request.method == "POST":
        stripe_customer = stripe.Customer.create(
            email=request.user.email, source=request.POST["stripeToken"]
        )
        plan = "price_1NcxW8CnWLQdzz97B9uG2Bka"
        if request.POST["plan"] == "yearly":
            plan = "price_1NcxfgCnWLQdzz97oe6MFQGU"
        ...
        if request.method == "GET" and "coupon" in request.GET:
            if request.GET["coupon"].lower() in coupons:
                ...
                coupon_price = int((int(percentage) / 100) * price)
